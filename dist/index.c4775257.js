(()=>{
    ...
    function startup() {
        ...
        boxCountInput.addEventListener("keydown", (event)=>{
            ...
            if (newCount != colorBox.childElementCount) colorBoxes = createColorBoxes(newCount);
        });
        setInterval(()=>{
            takepicture();
        }, 1000);
    ...
    function extendPalette(palette) {
        ...
        for(let index = 0; index < colorBoxes.length; index++){
            const paletteIndex = index >= palette.length ? index % palette.length : index;
    ...
    function getDisplayType() {
        ...
        const displayTypeInput = document.querySelector('input[name="displayType"]:checked');
        const displayType = displayTypeInput ? displayTypeInput.value : defaultDisplayType;
        return displayType;
    }
    function drawColours(image) {
        ...
        if (displayType === "sort") rgbColours = sort(extendedPalette);
        else if (displayType === "shuffle") rgbColours = shuffle(extendedPalette);
    ...
    function takepicture() {
        ...
        if (width && height) {
            ...
            if (photo.complete) {
                ...
                drawColours(photo);
            } else photo.addEventListener("load", function() {
                drawColours(photo);
            });
        } else clearphoto();
    }
